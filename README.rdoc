...
* Storing which user is responsible for a revision. Rather than introduce a lot of controller magic to guess what to store, you can simply update an additional attribute on your versioned model: <tt>updated_by_user</tt>.

    @user.update_attributes(:last_name => "Jobs", :updated_by_user => "Tyler")
    @user.versions.last.user # => "Tyler"

  Instead of passing a simple string to the <tt>updated_by_user</tt> setter, you can pass a model instance, such as an ActiveRecord user or administrator. The association will be saved polymorphically alongside the version.

    @user.update_attributes(:last_name => "Jobs", :updated_by_user => current_user)
